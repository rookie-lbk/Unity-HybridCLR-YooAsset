...
{
    internal static partial class FR2_Parser // UIToolkit
    {
            ...
            if (line.Contains("<Style") && line.Contains("src="))
            {
                ...
                if (!string.IsNullOrEmpty(srcPath))
                {
                    ...
                    if (!string.IsNullOrEmpty(resolvedPath))
                    {
                        ...
                        {
                            // FR2_LOG.Log($"[FR2] UXML->USS: {srcPath} -> {resolvedPath} -> {guid}");
                            return (guid, -1);
                        }
                    }
                    else
                    {
                        FR2_LOG.LogWarning($"[FR2] Failed to resolve UXML Style src: {srcPath}");
                    }
                }
            }
            ...
            if (line.Contains("@import"))
            {
                ...
                if (!string.IsNullOrEmpty(importPath))
                {
                    ...
                    if (!string.IsNullOrEmpty(resolvedPath))
                    {
                        ...
                        if (!string.IsNullOrEmpty(guid))
                        {
                            // FR2_LOG.Log($"[FR2] USS->USS: {importPath} -> {resolvedPath} -> {guid}");
                            return (guid, -1);
                        }
                    }
                    else
                    {
                        FR2_LOG.LogWarning($"[FR2] Failed to resolve USS import: {importPath}");
                    }
                }
            }
