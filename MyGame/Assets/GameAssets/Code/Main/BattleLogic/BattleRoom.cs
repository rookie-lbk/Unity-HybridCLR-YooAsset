...
public class BattleRoom
{
    ...
    public void UpdateRoom()
    {
        ...
        if (_steps == ESteps.Ready)
        {
            ...
            {
                // 生成实体
                var assetHandle = YooAssets.LoadAssetAsync<GameObject>("Entity/player_ship.prefab");
        ...
        if (_steps == ESteps.SpawnEnemy)
        {
            ...
            // 生成实体
            var assetHandle = YooAssets.LoadAssetAsync<GameObject>("Entity/" + enemyLocation + ".prefab");
            assetHandle.Completed += (AssetHandle handle) =>
            {
    ...
    private void OnHandleEventMessage(IEventMessage message)
    {
        if (message is BattleEventDefine.PlayerDead)
        {
            ...
            // 创建爆炸效果
            var assetHandle = YooAssets.LoadAssetAsync<GameObject>("Effect/explosion_player.prefab");
            assetHandle.Completed += (AssetHandle handle) =>
            {
        ...
        else if (message is BattleEventDefine.EnemyDead)
        {
            ...
            // 创建爆炸效果
            var assetHandle = YooAssets.LoadAssetAsync<GameObject>("Effect/explosion_enemy.prefab");
            assetHandle.Completed += (AssetHandle handle) =>
            {
                handle.InstantiateSync(msg.Position, msg.Rotation, _roomRoot.transform);
            };
        ...
        else if (message is BattleEventDefine.AsteroidExplosion)
        {
            ...
            // 创建爆炸效果
            var assetHandle = YooAssets.LoadAssetAsync<GameObject>("Effect/explosion_asteroid.prefab");
            assetHandle.Completed += (AssetHandle handle) =>
            {
                handle.InstantiateSync(msg.Position, msg.Rotation, _roomRoot.transform);
            };
        ...
        else if (message is BattleEventDefine.PlayerFireBullet)
        {
            ...
            // 创建子弹实体
            var assetHandle = YooAssets.LoadAssetAsync<GameObject>("Entity/player_bullet.prefab");
            assetHandle.Completed += (AssetHandle handle) =>
            {
                handle.InstantiateSync(msg.Position, msg.Rotation, _roomRoot.transform);
            };
        ...
        else if (message is BattleEventDefine.EnemyFireBullet)
        {
            ...
            // 创建子弹实体
            var assetHandle = YooAssets.LoadAssetAsync<GameObject>("Entity/enemy_bullet.prefab");
            assetHandle.Completed += (AssetHandle handle) =>
            {
                handle.InstantiateSync(msg.Position, msg.Rotation, _roomRoot.transform);
            };
